...
import type { KernelBusyAfterInterruptError } from "./kernel";
import type { KernelExecutionResult } from "./kernel";
 ...
 */
export function createRlmToolDefinition(
  options: RlmToolDefinitionOptions,
): {
  name: string;
  label: string;
  description: string;
  ...
  return {
    ...
    async execute(
      ...
      const phaseToMessage = (
        phase: "starting" | "restoring" | "preparing",
      ): string => {
      ...
      try {
        ...
        while (true) {
          try {
            ...
            if (
              ...
              error.name === "KernelBusyAfterInterruptError"
            ) {
        ...
        const kernelRestarted =
          toolKernelRestarted || result.kernelRestarted === true;
        toolKernelRestarted = false;

        // Text assembly
        let text = assembleText(
          result.stdout,
          result.stderr,
          result.result,
          result.traceback,
