...
export class KernelUnresponsiveError extends RlmKernelContractError {
	constructor(stderrTail: string) {
		super(`Kernel did not respond to kernel_info_request within ${READY_TIMEOUT_MS}ms. stderr tail: ${stderrTail}`);
...
export class KernelPortsUnresolvedError extends RlmKernelContractError {
	constructor(stderrTail: string) {
		super(`Kernel did not resolve connection ports within ${PORTS_RESOLVE_TIMEOUT_MS}ms. stderr tail: ${stderrTail}`);
...
function truncateStream(text: string): string {
	...
	return text.slice(0, MAX_OUTPUT_CHARS) + TRUNCATION_MARKER.replace("%d", String(MAX_OUTPUT_CHARS));
}
...
function boundedStderrTail(stderr: string): string {
	return stderr.length <= STDERR_TAIL_CHARS ? stderr : stderr.slice(stderr.length - STDERR_TAIL_CHARS);
}
...
export class KernelManager {
	...
	constructor(transport: KernelTransport, options: { clock: KernelClock; artifactsDir: string }) {
		...
		this.transport.onOutput((event: KernelOutputEvent) => {
			if (this.activeExecutionId !== null && event.id === this.activeExecutionId) {
	...
	execute(code: string): Promise<KernelExecutionResult> {
		if (this.disposed) {
			return Promise.reject(new RlmKernelContractError("KernelManager has been disposed"));
		}
	...
	async dispose(options?: { readonly snapshot?: boolean }): Promise<void> {
		...
		const waitTarget = this.chainTail ?? Promise.resolve();
		await Promise.race([
			waitTarget,
			new Promise<void>(resolve => {
	...
	private async waitForNotBusy(): Promise<void> {
		...
		while (this.transport.isBusy()) {
			...
			if (elapsed >= BUSY_REUSE_WAIT_MS) {
				throw new KernelBusyAfterInterruptError(`waited ${elapsed}ms`);
			}
			safeInterrupt(this.transport);
			await new Promise<void>(resolve => {
	...
	private async executeInternal(code: string): Promise<KernelExecutionResult> {
	...
	private buildResult(execResult: KernelTransportExecuteResult, durationMs: number): KernelExecutionResult {
		// POST-KM-1: transport code !== 0 → status 'error'
		const status: KernelExecutionStatus = execResult.code !== 0 ? "error" : "ok";
	...
	private async runSnapshot(): Promise<void> {
		...
		const run = (async (): Promise<void> => {
			...
			const filteredNames = allNames.filter((n: string) => !skipSet.has(n));

			// Write snapshot (serialization in kernel process); C2/F-174: the
			// byte cap flows through the seam — the writer is TOLD the budget
			const writeResult = await this.transport.writeSnapshot(filteredNames, SNAPSHOT_MAX_BYTES);
			...
			const pythonVersion = await (this.transport.pythonVersion?.() ?? Promise.resolve(""));
			...
			atomicWriteFileSync(join(this.artifactsDir, SNAPSHOT_MANIFEST_FILE), JSON.stringify(manifest));
		})();
	...
	async onCompactionComplete(): Promise<void> {
		...
		this.compactionNoticeValue = `Kernel state persisted. Namespace inventory: ${liveNames.join(", ")}`;
	}
