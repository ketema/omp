...
- terminal/credential prompts reduced (`TERM=dumb`, `GIT_TERMINAL_PROMPT=0`, `NO_COLOR=1`, `CI=true` unless `PI_BASH_NO_CI`/`CLAUDE_BASH_NO_CI` is set); `SSH_ASKPASS` keeps a real askpass when present (YubiKey/ed25519-sk), else `/usr/bin/false`,
